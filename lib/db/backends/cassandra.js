...
CassandraClient.prototype._execute = function(query, args, callback) {
  ...
  // Before executing a query, wait for connection process to finish.
  async.until(function truthTest() {
    return self._connected;
  },

  function waitForConnect(callback) {
    log.debug('Waiting for connect process to finish before executing' +
              ' the first query...');

    setTimeout(callback, CONNECT_WAIT_DELAY);
  },

  function whenDoneExecuteQuery(err) {
    self._client.execute(query, args, function(err, res, metadata) {
      if (err) {
        callback(err);
        return;
      }
