...
var US_AUTH_SERVICE_URL = 'https://auth.api.rackspacecloud.com/v2.0';
var UK_AUTH_SERVICE_URL = 'https://lon.auth.api.rackspacecloud.com/v2.0';
...
function verifyToken(tenantId, token, callback) {
  var clientParams = [{'url': US_AUTH_SERVICE_URL, 'options': {}}, {'url': UK_AUTH_SERVICE_URL, 'options': {}}],
      result = {};
  ...
  async.some(clientParams, function(params, callback) {
    ...
    client.validateTokenForTenant(tenantId, token, function(err, data) {
      ...
      if (!err && data) {
        log.debug('Auth service returned that a token is valid', {'url': url});
