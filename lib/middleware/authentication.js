...
exports.processRequest = function(req, res, callback) {
  var settings = config.middleware.authentication.settings,
      tenantId = req.userId, token = req.headers['x-auth-token'], cacheKey,
      pathName = url.parse(req.url).pathname;

  if (settings.whitelist.indexOf(pathName) !== -1) {
  ...
  if (!tenantId) {
    httpUtil.returnError(res, 400, 'Missing X-Tenant-Id header');
