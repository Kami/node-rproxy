...
function verifyToken(req, tenantId, token, callback) {
  ...
  async.some(clientParams, function(params, callback) {
    ...
    if (req.trace) {
      t = req.trace.child('validate token');
      ep = new req.tryfer.trace.Endpoint(req.trace.endpoint.ipv4,
                                         req.trace.endpoint.port,
                                         sprintf('%s:rproxy:auth', req.serviceNamePrefix));

      t.setEndpoint(ep);
      t.record(req.tryfer.trace.Annotation.clientSend());
      t.record(req.tryfer.trace.Annotation.string('rax.auth_url', params.url));
    }

    client.validateTokenForTenant(tenantId, token, function(err, data) {
      if (req.trace) {
        t.record(req.tryfer.trace.Annotation.clientRecv());
      }
