...
exports.processRequest = function(req, res, callback) {
  req.trace = trace.Trace.fromRequest(req, exports.SERVICE_NAME);

  callback();

  // Asynchronously record that we have received a client request
  req.trace.record(trace.Annotation.serverRecv());
  req.trace.record(trace.Annotation.string('request_headers',
                                   JSON.stringify(req.headers)));
  req.trace.record(trace.Annotation.string('user_id', req.userId));
};
