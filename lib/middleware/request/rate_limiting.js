...
function getUsage(id, callback) {
  ...
  async.forEach(settings.limits, function(rule, callback) {
    async.waterfall([
      ...
      function formatResult(keys, usage, bucketNameToValueMap, callback) {
        result.push({'method': rule.method, 'path_regex': rule.path_regex,
                     'limit': rule.limit, 'used': usage, 'period': rule.period});
        callback();
      }
    ], callback);
  },
...
exports.processRequest = function(req, res, callback) {
  var settings = config.middleware.rate_limiting, method = req.method,
      reachedLimitErrors = [], parsed = url.parse(req.url), pathName = parsed.pathname;
  ...
  async.waterfall([
    function getLimits(callback) {
      ...
      getUsage(req.userId, function(err, data) {
        if (err) {
          callback(err);
          return;
        }
    ...
    function checkAndUpdateLimits(callback) {
      if (req.trace) {
        t = req.trace.child('check and update limits');
        tryfer = req.tracing.tryfer;
        ep = new tryfer.trace.Endpoint(req.trace.endpoint.ipv4,
                                       req.trace.endpoint.port,
                                       sprintf('%s:rproxy:rate_limiting', req.tracing.prefix));

        t.setEndpoint(ep);
        t.record(tryfer.trace.Annotation.clientSend());
        t.record(tryfer.trace.Annotation.string('backend', config.database.backend));
      }
      ...
      function(err) {
        if (req.trace) {
          t.record(tryfer.trace.Annotation.clientRecv());
        }
