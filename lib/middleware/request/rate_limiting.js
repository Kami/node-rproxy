...
function checkAndUpdateLimit(id, rule, callback) {
  ...
  async.waterfall([
    ...
    function checkUsageAgainstLimit(keys, usage, bucketNameToValueMap, callback) {
      currentUsage = usage;
...
exports.processRequest = function(req, res, callback) {
  var settings = config.middleware.rate_limiting, method = req.method,
      reachedLimitErrors = [], pathName = url.parse(req.url).pathname;
  ...
  async.waterfall([
    function getLimits(callback) {
      if (pathName === settings.usage_proxy_path && req.method === 'GET') {
        // User requested a special URL, send raw usage data to the backend
        log.debug('User requested special usage path, sending usage to the backend...');
        ...
        getUsage(req.userId, function(err, data) {
          ...
          req.method = 'POST';
          req.url = settings.usage_target_path;
    ...
    function checkAndUpdateLimits(callback) {
      async.forEach(settings.limits, function(rule, callback) {
        ...
        if (rule.method === method && regex.test(req.url)) {
